...
import { buildSearchDocuments, extractSearchContent } from './search-index.mjs';

const DEFAULTS = { type: 'doc', draft: false, order: 0, tags: [] };

function metadataList(value) {
  const values = Array.isArray(value) ? value : value == null ? [] : [value];
  return [...new Set(values.map((item) => String(item ?? '').normalize('NFKC').trim().replace(/\s+/gu, ' ')).filter(Boolean))];
}
...
export async function generateContent(root = process.cwd()) {
  ...
  for (const dir of spaceDirs) {
    ...
    for (const file of files) {
      ...
      if (Boolean(data.draft) || Boolean(data.hidden)) continue;
      ...
      const directoryRoute = `/spaces/${slug}${slugify(dirParts.join('/')) ? `/${slugify(dirParts.join('/'))}` : ''}`;
      const parentDirParts = dirParts.slice(0, -1);
      const parentRoute = isIndex
        ? `/spaces/${slug}${slugify(parentDirParts.join('/')) ? `/${slugify(parentDirParts.join('/'))}` : ''}`
        : directoryRoute;
      const searchable = extractSearchContent(parsed.content, { mdx: /\.mdx$/i.test(file) });
      entries.push({
        ...
        description: String(data.description ?? searchable.text.slice(0, 160)), type: String(data.type ?? 'doc'),
        tags: metadataList(data.tags),
        date: data.date ? String(data.date) : '', order: Number(data.order ?? 0), draft: false,
        dirParts, headings: searchable.headings, body: rewriteMarkdownAssets(parsed.content, slug, file, spaceRoot), updatedAt: stat.mtime.toISOString(),
        directoryRoute,
        parentRoute,
      });
    }
  ...
  const searchOutput = `/* generated by scripts/content.mjs */\nexport const searchDocuments = ${JSON.stringify(buildSearchDocuments(spaces), null, 2)} as const;\n`;
  const searchDestination = path.join(generatedDir, 'search.ts');
